...
// ---------------- Algo Desk: real data (replaces the standalone simulated backend) ----------------
// Pulls from the SAME analyze() pipeline as every other terminal — same live Angel One
// session, same Greeks/dealer-exposure math. Only NIFTY and SENSEX are wired here because
// those are the only entries in INDEXES right now. BANKNIFTY needs its NSE index token
// added to INDEXES first (I didn't want to hardcode an unverified token). XAUUSD is a
// forex/gold pair — Angel One's retail API doesn't expose spot XAUUSD the way it does NSE/BSE
// indices, so that one likely needs a different data source entirely, not just a config tweak.
app.get('/api/desk-data', async (req, res) => {
  const out = [];
  const errors = [];
  let indiaVix = null;
  try { const v = await getVIX(); indiaVix = v != null ? +v.toFixed(2) : null; } catch (e) { errors.push('VIX: ' + e.message); }

  for (const idx of Object.keys(INDEXES)) {
    try {
      const data = await analyze(idx, null, {});
      const de = data.dealerExposure || {};
      const atmRow = (data.chain || []).find(r => r.strike === data.atm && r.type === 'CE')
                  || (data.chain || []).find(r => r.strike === data.atm);
      const oiReal = data.oiSource === 'NSE'; // OI is only genuinely real when the NSE merge succeeded
      const ceOI = oiReal ? (data.chain || []).filter(r => r.type === 'CE').reduce((s, r) => s + (r.oi || 0), 0) : null;
      const peOI = oiReal ? (data.chain || []).filter(r => r.type === 'PE').reduce((s, r) => s + (r.oi || 0), 0) : null;
      const totalVol = (data.chain || []).reduce((s, r) => s + (r.volume || 0), 0);
      out.push({
        asset: idx,
        timestamp: Date.now(),
        price: data.spot,
        iv: atmRow ? +(atmRow.iv / 100).toFixed(4) : (data.atmIV ? +(data.atmIV / 100).toFixed(4) : null),
        volume: totalVol,
        openInterest: oiReal ? (ceOI + peOI) : null,
        pcr: oiReal && peOI != null && ceOI ? +(peOI / ceOI).toFixed(2) : null,
        greeks: atmRow ? { delta: atmRow.delta, gamma: atmRow.gamma, vega: atmRow.vega, theta: atmRow.theta } : null,
        dealerPressure: de.dealerHedgePressureScore != null ? de.dealerHedgePressureScore : null,
        // Walls are only meaningful once oiSource is 'NSE' — otherwise they're volume-proxy
        // walls (still shown, but flagged) rather than true OI walls.
        callWall: de.callWall != null ? de.callWall : null,
        putWall: de.putWall != null ? de.putWall : null,
        wallBasis: oiReal ? 'OI' : (data.dataSource === 'BROKER_IV' ? 'VOLUME_PROXY' : 'UNAVAILABLE'),
        expiryUsed: data.expiry,
        indiaVix,
        dataSource: data.dataSource,   // 'BROKER_IV' = real Angel One Greeks; 'VIX_PROXY_IV...' = synthetic fallback, broker fetch failed
        oiSource: data.oiSource,       // 'NSE' = real OI merge worked; 'NONE' = not attempted/failed (SENSEX never attempts NSE OI)
        real: data.dataSource === 'BROKER_IV'
      });
    } catch (e) {
  ...
  res.json({
    success: true, generatedAt: new Date().toISOString(), data: out, errors,
    note: "dataSource=VIX_PROXY_IV means the broker Greeks fetch failed for that index and this is a synthetic fallback, not live data. oiSource=NONE means OI/PCR are genuinely unavailable (shown as null, not zero) — currently only NIFTY even attempts the NSE OI merge, and that merge can fail if NSE blocks the server's IP."
  });
});
