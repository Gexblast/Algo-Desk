...
app.get('/api/desk-data', async (req, res) => {
  ...
  for (const idx of Object.keys(INDEXES)) {
    try {
      ...
      const ceOI = (data.chain || []).filter(r => r.type === 'CE').reduce((s, r) => s + (r.oi || 0), 0);
      const peOI = (data.chain || []).filter(r => r.type === 'PE').reduce((s, r) => s + (r.oi || 0), 0);
      ...
      out.push({
        ...
        pcr: data.pcr != null ? data.pcr : null,
        real: true
      });
    } catch (e) {
  ...
  res.json({ success: true, generatedAt: new Date().toISOString(), data: out, errors, note: 'Live data via Angel One SmartAPI — same pipeline as Wall Sniper/Gann/Playbook. BANKNIFTY/XAUUSD not wired yet.' });
});
